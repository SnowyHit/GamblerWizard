  ...
  return (
    <div className={`flex flex-col gap-1.5 ${width} max-w-xs`}>
      <div className="flex justify-between items-end px-0.5">
        <span className="text-[8px] text-slate-200 uppercase font-bold tracking-widest">{label}</span>
        <span className="text-[7px] text-slate-400 font-mono tracking-tighter">{Math.ceil(current)} / {max}</span>
      </div>
